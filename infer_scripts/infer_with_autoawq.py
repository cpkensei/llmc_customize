...
# Configure model path and output directory
# MODEL_PATHS = ['/root/autodl-tmp/model_save/gptq/autoawq_quant_model_Meta-Llama-3-8B_4']
MODEL_PATHS = glob('/root/autodl-tmp/model_save/llama3_all/autoawq*')
...
def init_pynvml():
    try:
        ...
        print(f"Debug: Error initializing pynvml: {e}")
        return None

# Get current VRAM usage
def get_vram_usage(handle, baseline_vram=0.0):
...
def measure_ttft(model, tokenizer, prompt, sampling_params, vram_samples, baseline_vram, handle, device):
    ...
    try:
        ...
        def on_token(token):
            if not first_token_received[0]:
                ...
                vram_samples.append(get_vram_usage(handle, baseline_vram))
...
def monitor_vram(vram_samples, stop_flag, baseline_vram, handle):
...
def cleanup_processes():
    """Kill residual AutoAWQ-related processes to free GPU memory."""
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if any('python' in str(arg).lower() and 'awq' in str(arg).lower() for arg in cmdline):
...
if __name__ == "__main__":
    ...
    for model_path in MODEL_PATHS:
        ...
        # Initialize pynvml
        handle = init_pynvml()
        baseline_vram = get_vram_usage(handle) if handle else 0.0
        ...
        for prompt in prompts:
            vram_samples = []
            stop_flag = [False]
            vram_thread = Thread(target=monitor_vram, args=(vram_samples, stop_flag, baseline_vram, handle))
            vram_thread.start()

            start_time = time.time()
            ttft = measure_ttft(model, tokenizer, prompt, sampling_params, vram_samples, baseline_vram, handle, device)
            if ttft is None:
